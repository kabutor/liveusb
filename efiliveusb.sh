...
rm -f liveusb.img
truncate -s 500M liveusb.img
parted liveusb.img <<EOF
mklabel gpt
mkpart primary fat32 1MiB 260MiB
set 1 esp on
mkpart primary ntfs 261MiB 100%
quit
EOF
...
sudo mkfs.exfat -n ISOs /dev/loop5p2
...
sudo grub-install --target=x86_64-efi --removable --efi-directory=/tmp/liveusb/ --boot-directory=/tmp/liveusb/ 
